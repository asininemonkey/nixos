...
echo password | cryptsetup luksFormat --batch-mode --cipher aes-xts-plain64 --hash sha512 --key-size 512 --type luks1 "${DEVICE}2"
echo password | cryptsetup luksAddKey --batch-mode "${DEVICE}2" /tmp/crypto_keyfile.bin

cryptsetup open --batch-mode --key-file /tmp/crypto_keyfile.bin --type luks1 "${DEVICE}2" crypt-nixos

mkfs.fat -n boot -F 32 "${DEVICE}1"
...
sed --in-place 's/xxx/qemu/' /mnt/etc/nixos/configuration.nix
sed --in-place "s/xxx/$(blkid --match-tag UUID --output value ${DEVICE}2)/" /mnt/etc/nixos/crypt.nix
